...
export default function DemoPage() {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          try {
            const errorData = await response.json()
            if (response.status === 401) {
              alert(errorData.message || 'Invalid password. Please check your password and try again.')
            } else {
  ...
  const handleRemoveFile = async (fileId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/demo/upload/${fileId}?password=${encodeURIComponent(password)}`, {
        method: 'DELETE',
      })
      ...
      <aside className="knowledge-base">
        ...
        <div className="upload-section">
          <input
            ...
            hidden
          />
          <label htmlFor="file-upload" className={`upload-label ${uploadingFile ? 'uploading' : ''}`}>
            <span>{uploadingFile ? `Uploading ${uploadingFile}...` : '+ Add Document'}</span>
          </label>
        </div>
