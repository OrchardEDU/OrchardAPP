...
export class Generator {
    constructor(qdrantUrl = null) {
        ...
        console.log(`RAG Generator initialized with Qdrant (starting point ID: ${this._pointIdCounter})`);
    }
    ...
    async _ensureCollection() {
        ...
        try {
            ...
            throw new Error(`Collection ${this.collectionName} does not exist in Qdrant: ${error.message}`);
        }
    }
    ...
    async search(queryText, limit = 5, userId = null, subjectId = null) {
        ...
        console.log(`[Qdrant] Successfully retrieved ${results.length} results from Qdrant for query: "${queryPreview}"`);
        return results;
    }
    ...
    async retrieveContext(queryText, limit = 5, userId = null, subjectId = null) {
        ...
        console.log(`[Qdrant] Successfully retrieved ${contextChunks.length} context chunks from Qdrant`);
...
Generate the question now:`;
            ...
            console.log(`[RAG] Generated question for prompt "${queryText}": ${generatedQuestion}`);
    ...
    async addDocument(fileId, text, filename, userId = null, subjectId = null) {
        try {
            ...
            console.log(`[Qdrant] Uploading ${points.length} points to Qdrant for file: ${filename} (IDs: ${startId}-${endId})...`);
            ...
            console.log(`[Qdrant] Successfully uploaded ${points.length} chunks to Qdrant for file: ${filename} (point IDs: ${startId}-${endId})`);
